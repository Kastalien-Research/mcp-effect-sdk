...
export class Annotations extends Schema.Struct({
   ...
   * string (e.g., "2025-01-12T15:00:58Z").
   */
...
export class Icon extends Schema.Struct({
  /**
   * A standard URI pointing to an icon resource. May be an HTTP/HTTPS
   * URL or a `data:` URI with Base64-encoded image data.
   */
  src: Schema.String,
  /**
  ...
  /**
   * Optional sizes in WxH format (e.g., "48x48") or "any" for SVG.
   */
...
export class Implementation extends Schema.Struct({
  ...
  /**
   * A short description of this implementation.
   */
  description: Schema.optional(Schema.String),
  /**
   * Optional set of sized icons for this implementation.
   */
  icons: Schema.optional(Schema.Array(Icon)),
  /**
   * An optional URL for the implementation's website.
   */
...
export class ClientCapabilities extends Schema.Class<ClientCapabilities>(
  ...
  sampling: Schema.optional(Schema.Struct({
    /**
     * Present if the client supports tool use in sampling.
     */
    tools: Schema.optional(Schema.Struct({})),
    /**
     * Present if the client supports context inclusion (soft-deprecated).
     */
...
export class Resource extends Schema.Class<Resource>(
  ...
  _meta: Schema.optional(Schema.Record({
    key: Schema.String,
  ...
  /**
   * Optional set of sized icons for this resource.
   */
  icons: Schema.optional(Schema.Array(Icon)),
  /**
...
export class ResourceTemplate extends Schema.Class<ResourceTemplate>(
  ...
  annotations: Schema.optional(Annotations),
  /**
   * Optional set of sized icons for this resource template.
   */
  icons: Schema.optional(Schema.Array(Icon))
}) {}
...
export class Prompt extends Schema.Class<Prompt>(
  ...
  /**
   * Optional set of sized icons for this prompt.
   */
  icons: Schema.optional(Schema.Array(Icon))
}) {}
...
export class ToolUseContent extends Schema.Struct({
  ...
  /**
   * A unique identifier for this tool use, used to match results.
   */
  id: Schema.String,
  /**
  ...
  /**
   * Optional metadata. Clients SHOULD preserve this field when
   * including tool uses in subsequent sampling requests.
   */
  _meta: Schema.optional(Schema.Record({
    key: Schema.String,
    value: Schema.Unknown
...
export class ToolResultContent extends Schema.Struct({
  ...
  /**
   * Optional metadata. Clients SHOULD preserve this field.
   */
  _meta: Schema.optional(Schema.Record({
    key: Schema.String,
    value: Schema.Unknown
...
/**
 * A content block used in tool results, prompt messages, and resource content.
 *
 * Per the MCP 2025-11-25 spec, this union does NOT include ToolUseContent
 * or ToolResultContent — those belong in SamplingMessageContentBlock.
 *
 * @since 1.0.0
 * @category Prompts
 */
...
/**
 * A content block used in sampling messages and create-message results.
 *
 * Per the MCP 2025-11-25 spec, this union includes ToolUseContent and
 * ToolResultContent (for tool-augmented sampling) but NOT EmbeddedResource
 * or ResourceLink.
 *
 * @since 1.0.0
 * @category Sampling
...
export class ToolExecution extends Schema.Struct({
   ...
   *
   * - "forbidden": does not support tasks (default when absent)
...
export class SamplingMessage extends Schema.Struct({
  ...
  role: Role,
  /**
   * Content of the message. Can be a single content block or an array
   * of SamplingMessageContentBlock items.
   */
...
export class CreateMessageResult extends Schema.Class<CreateMessageResult>(
  ...
  /**
   * The content of the generated message. Can be a single content block
...
export class CreateMessage extends Rpc.make("sampling/createMessage", {
  ...
  payload: {
    ...
    /**
     * Tools the LLM may use during sampling. Per the 2025-11-25 spec,
     * these are full Tool definitions (not a reduced type).
     */
...
// ---------------------------------------------------------------------------
// Elicitation Form Schema Types
// ---------------------------------------------------------------------------

/**
 * A string field in an elicitation form.
 *
 * @since 1.0.0
 * @category Elicitation
 */
export class ElicitStringSchema extends Schema.Struct({
  type: Schema.Literal("string"),
  title: Schema.optional(Schema.String),
  description: Schema.optional(Schema.String),
  default: Schema.optional(Schema.String),
  format: Schema.optional(
    Schema.Literal("date", "date-time", "email", "uri")
  ),
...
/**
 * A number/integer field in an elicitation form.
 *
 * @since 1.0.0
 * @category Elicitation
 */
export class ElicitNumberSchema extends Schema.Struct({
  type: Schema.Union(Schema.Literal("number"), Schema.Literal("integer")),
  title: Schema.optional(Schema.String),
  description: Schema.optional(Schema.String),
...
/**
 * A boolean field in an elicitation form.
 *
 * @since 1.0.0
 * @category Elicitation
 */
export class ElicitBooleanSchema extends Schema.Struct({
...
/**
 * A single-select enum field with untitled options.
 *
 * @since 1.0.0
 * @category Elicitation
 */
export class ElicitUntitledEnumSchema extends Schema.Struct({
  type: Schema.Literal("string"),
  title: Schema.optional(Schema.String),
  description: Schema.optional(Schema.String),
  default: Schema.optional(Schema.String),
...
/**
 * A single-select enum field with titled options (oneOf).
 *
 * @since 1.0.0
 * @category Elicitation
 */
export class ElicitTitledEnumSchema extends Schema.Struct({
  type: Schema.Literal("string"),
  title: Schema.optional(Schema.String),
  description: Schema.optional(Schema.String),
  default: Schema.optional(Schema.String),
...
/**
 * A multi-select enum field with untitled options.
 *
 * @since 1.0.0
 * @category Elicitation
 */
export class ElicitUntitledMultiEnumSchema extends Schema.Struct({
...
/**
 * A multi-select enum field with titled options.
 *
 * @since 1.0.0
 * @category Elicitation
 */
export class ElicitTitledMultiEnumSchema extends Schema.Struct({
  type: Schema.Literal("array"),
  title: Schema.optional(Schema.String),
  description: Schema.optional(Schema.String),
  default: Schema.optional(Schema.Array(Schema.String)),
...
/**
 * Union of all primitive schema definitions allowed in elicitation forms.
 *
 * @since 1.0.0
 * @category Elicitation
 */
export const PrimitiveSchemaDefinition = Schema.Union(
  // Enum schemas first — they have extra required fields (enum/oneOf/items)
  // that distinguish them from plain string. Schema.Union tries in order.
  ElicitTitledEnumSchema,
  ElicitUntitledEnumSchema,
  ElicitTitledMultiEnumSchema,
  ElicitUntitledMultiEnumSchema,
  // Primitives last (least specific)
  ElicitBooleanSchema,
  ElicitNumberSchema,
  ElicitStringSchema
)
...
// ---------------------------------------------------------------------------
// Elicitation Request Modes
// ---------------------------------------------------------------------------

/**
 * Form-mode elicitation payload. The server requests structured input from
 * the user via a restricted JSON Schema form.
 *
 * @since 1.0.0
 * @category Elicitation
 */
...
/**
 * URL-mode elicitation payload. The server provides a URL for the client
 * to open, typically for OAuth or external authentication flows.
 *
 * @since 1.0.0
 * @category Elicitation
 */
export class ElicitUrlPayload extends Schema.Struct({
  ...
  /**
   * A unique identifier for this elicitation request.
   */
  ...
  message: Schema.String,
  /**
   * The URL for the client to open.
   */
  url: Schema.String,
  /**
   * Optional task metadata for task-augmented execution.
 ...
 * The response structure matches the result type of the original request.
 *
 * @since 1.0.0
 * @category Tasks
 */
 ...
 * them that a task's status has changed.
 *
 * @since 1.0.0
 * @category Tasks
 */

...
if (!apiKey) {
  // Fall back to the airtable MCP server's env file, which is the canonical key location.
  // Resolved from this script's location: .claude/scripts/ -> app root -> servers/.
  const envPath = new URL("../../servers/effect-airtable-mcp/.env", import.meta.url).pathname
  try {
...
if (!apiKey) {
  console.error("AIRTABLE_API_KEY not set and not found in servers/effect-airtable-mcp/.env")
  process.exit(2)
}

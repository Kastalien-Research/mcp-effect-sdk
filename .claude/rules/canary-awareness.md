...
The Canary is an ambient monitoring system that tracks session pressure through
edit velocity, scope breadth, turn count, and elapsed time. Signals are written
to `.claude/state/canary_signals.json` by the `10_canary_monitor.py` hook.

### Reading Canary Signals

At key decision points — before expanding scope, before new explorations, before
adding features — check the canary:
...
- **> 0.7**: Red. Strongly consider phase transition toward completion. Use
  `/helm` to evaluate.
...
1. **Never ignore a canary warning.** Acknowledge it and explain why you're
   proceeding.
...
The Canary doesn't make decisions — it provides pressure signals. The Helmsman
(`/helm`) interprets them in context.

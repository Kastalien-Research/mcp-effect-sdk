...
When writing technical specifics — package names, CLI flags, API signatures,
configuration formats, endpoint URLs — **verify against reality before writing
them into code or config.**
...
"I'm pretty sure it's X" is not verification. If you can't verify, say so and
help the user find the answer — don't guess.
...
- Takes multiple round trips to diagnose because the error points away from the
  root cause

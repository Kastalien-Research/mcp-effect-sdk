...
writes, emails, and git push. All six smoke checks were green for two days.
The first live intake after cutover then hit, in sequence:
...
- McpExecutor treated HTTP-200 + `isError: true` tool results as SUCCESS, so
  the dead write ALSO reported green and the chain marched on (fixed: tool
  errors throw, 42213df).
...
the dry-run flag suppresses (grep for the flag; each `if (isDryRun())` branch
is one) and exercise the REAL path for each against a scratch target:
...
system. The suppressed half is the half that talks to the outside world —

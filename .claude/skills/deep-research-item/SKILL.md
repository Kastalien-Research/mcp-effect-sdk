...
description:
  Run comprehensive research on an entity — news, funding, tech stack, buyer
  mapping, angle building. Writes findings to the webset store and optionally to
  Airtable. Use after verification passes or when a channel event requires deep
  research on a specific entity.
argument-hint: [entityName domain websetId]
user-invocable: true
allowed-tools:
  Read, mcp__websets-codemode-local__execute,
  mcp__websets-codemode-local__search, mcp__airtable__execute
---
...
For the top 3-5 most relevant URLs from step 1, run `exa.getContents` to extract
full text. Focus on:
...
If the workflow config includes an Airtable target, upsert the research brief
using the Airtable Code Mode execute tool.

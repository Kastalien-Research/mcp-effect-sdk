...
const makeMemoryStore = (): Auth.AuthorizationClientStoreService => {
  ...
  return {
    findCredential: (key) =>
      Effect.sync(() => {
        for (const [candidate, credential] of credentials) {
          if (credential.issuer === key.issuer && (key.clientId === undefined || credential.clientId === key.clientId))
    ...
    readCredential: (saved) =>
      credentials.has(saved) ? Effect.succeed(credentials.get(saved)!) : failStore("readCredential"),
    findGrant: (key) =>
      Effect.sync(() => {
        for (const [candidate, grant] of grants) {
          if (
            grant.issuer === key.issuer &&
            grant.resource === key.resource &&
            grant.clientId === key.clientId &&
            sameScopes(grant.scopes, key.scopes)
          )
            return Option.some(candidate)
        }
        return Option.none()
    ...
    readGrant: (saved) => (grants.has(saved) ? Effect.succeed(grants.get(saved)!) : failStore("readGrant")),
    removeGrant: (saved) =>
      Effect.sync(() => {
        grants.delete(saved)
      }),
    ...
    Effect.tryPromise({
      try: async () => {
        ...
        const response = await fetch(request.url, {
          ...
          headers,
          redirect: "manual",
          ...(request.body === undefined ? {} : { body: Buffer.from(Redacted.value(request.body)) })
        })
        return {
          status: response.status,
          headers: Object.freeze(
            [...response.headers.entries()].map(([name, value]) => Object.freeze([name, Redacted.make(value)] as const))
          ),
          body: Redacted.make(new Uint8Array(await response.arrayBuffer()))
        }
      },
      catch: () =>
        new Auth.AuthorizationHttpError({
          operation: "request",
          retryable: false
        })
    })
    ...
    waitForCallback: (request) =>
      Effect.try({
        try: () => {
          ...
          new Auth.AuthorizationInteractionError({
...
const runDraftToolsCallClient = (serverUrl: string): Promise<void> =>
  withClient(serverUrl, { name: "draft-tools-call-client" }, (client) =>
    Effect.gen(function* () {
      const tools = yield* client.listTools()
      assert(
        tools.tools.some(({ name }) => name === "test_simple_text"),

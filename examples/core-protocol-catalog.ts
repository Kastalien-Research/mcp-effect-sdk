...
import * as McpClientApi from "mcp-effect-sdk/client"
import { McpProtocol, McpSchema } from "mcp-effect-sdk/protocol/2026-07-28"
...
export const resourceWorkspaceClient = (client: McpClient.McpClient): Effect.Effect<void, unknown, unknown> =>
  Effect.scoped(
    Effect.gen(function* () {
...
export const promptPackLayer = Layer.mergeAll(
  ...
  McpServer.prompt({
    ...
    content: () => Effect.succeed([promptMessage(image()), promptMessage(text("Describe the image content."))])
  })
)
...
export const completionLayer = Layer.mergeAll(
  McpServer.prompt({
    ...
    completion: {
      component: (input) => Effect.succeed(["client", "server", "transport"].filter((value) => value.startsWith(input)))
    },
...
export const runCompletionClient = (client: McpClient.McpClient): Effect.Effect<void, unknown, never> =>
  Effect.gen(function* () {

...
import type { McpErrors } from "mcp-effect-sdk/protocol/2026-07-28"
...
const jsonResource = (uri: string, value: unknown): typeof McpSchema.ReadResourceResult.Type =>
...
const validTraceKinds = new Set<TraceEventKind>(["offered", "selected", "ignored", "retried", "failed"])

const isTraceEventKind = (value: string): value is TraceEventKind => validTraceKinds.has(value as TraceEventKind)
...
const registerDiscoverAndChooseGoal = (): Effect.Effect<void, SchemaValidationError, McpServer.McpServer> =>
  McpServer.registerResource({
    ...
    content: Effect.succeed(
      jsonResource("eval://discover-and-choose/goal", {
...
const registerDiscoverAndChooseAffordances = (): Effect.Effect<void, SchemaValidationError, McpServer.McpServer> =>
  Effect.gen(function* () {
    ...
    yield* McpServer.registerTool({
      ...
      content: ({ project }) =>
        Effect.succeed([text(`Launch checklist for ${project}: owner, rollback plan, metrics, comms.`)])
    })
  })
...
const registerAmbiguousAffordanceBrief = (): Effect.Effect<void, SchemaValidationError, McpServer.McpServer> =>
  McpServer.registerResource({
    ...
    priority: 1,
    content: Effect.succeed(
      jsonResource("eval://ambiguous-affordance/brief", {
...
const registerRecoveryBrief = (): Effect.Effect<void, SchemaValidationError, McpServer.McpServer> =>
  McpServer.registerResource({
    ...
    priority: 1,
    content: Effect.succeed(
      jsonResource("eval://recovery/case", {
...
const registerResourceFirstContext = (): Effect.Effect<void, SchemaValidationError, McpServer.McpServer> =>
  McpServer.registerResource({
    ...
    priority: 1,
    content: Effect.succeed(
      jsonResource("eval://resource-first/policy", {
  ...
  McpServer.registerTool({
    ...
    content: ({ service, policyVersion }) => {
      if (policyVersion !== "deploy-2026-05") {
        return Effect.succeed(toolError("Read eval://resource-first/policy and retry with its policyVersion."))
      }
...
const registerPromptOrToolBrief = (): Effect.Effect<void, SchemaValidationError, McpServer.McpServer> =>
  McpServer.registerResource({
    ...
    priority: 1,
    content: Effect.succeed(
      jsonResource("eval://prompt-or-tool/brief", {
  ...
  Effect.gen(function* () {
    yield* McpServer.registerPrompt({
      ...
      content: ({ incidentId, impact }) =>
        Effect.succeed([promptMessage(`Draft a concise incident update for ${incidentId}. Impact: ${impact}.`)])
    })
    yield* McpServer.registerTool({
...
const registerObservabilityBrief = (): Effect.Effect<void, SchemaValidationError, McpServer.McpServer> =>
  McpServer.registerResource({
    ...
    priority: 1,
    content: Effect.succeed(
      jsonResource("eval://observability/brief", {

...
const runGenerateMcp = Effect.sync(() => {
  ...
  const schemaOutputPath = path.join(root, "src/generated/mcp", protocolVersion, "McpSchema.generated.ts")

  const checkOnly = process.argv.includes("--check")

  const schemaJsonPath = path.join(root, schemaJsonSource.vendoredPath)
  const schemaTsPath = path.join(root, schemaTsSource.vendoredPath)

  const schemaJsonBytes = readFileSync(schemaJsonPath)
  const schemaTsBytes = readFileSync(schemaTsPath)
  assertPinnedSource(schemaJsonPath, schemaJsonBytes, schemaJsonSource.sha256)
  assertPinnedSource(schemaTsPath, schemaTsBytes, schemaTsSource.sha256)
  ...
  // The 2026-07-28 protocol gives every client request a concrete
  ...
  if (declaredProtocolVersion !== protocolVersion) {
    throw new Error(
      `${relative(schemaTsPath)} declares ${declaredProtocolVersion}, but ${relative(manifestPath)} selects ${protocolVersion}`
    )
  }
  ...
  // The stateless 2026-07-28 protocol has no server-initiated requests: server→client
  ...
  if (changed && checkOnly) {
    throw new Error(`Generated file is out of date and --check was requested.`)
  }
  ...
  if (existsSync(obsoleteProtocolOutputPath)) {
    if (checkOnly) {
      ...
      throw new Error(`Obsolete generated file must be removed: ${relative(obsoleteProtocolOutputPath)}`)
    }
  ...
  function readManifestProtocolVersion(value) {
    if (typeof value?.protocolVersion !== "string" || value.protocolVersion.length === 0) {
      throw new Error(`${relative(manifestPath)} must declare a protocolVersion`)
    }
  ...
  function readManifestSchemaFile(source, fileName) {
    const upstreamPath = `schema/${protocolVersion}/${fileName}`
    const vendoredPath = path.relative(root, path.join(sourceDir, fileName)).replaceAll(path.sep, "/")
    const matches = Array.isArray(source?.files)
      ? source.files.filter((file) => file?.upstreamPath === upstreamPath && file?.vendoredPath === vendoredPath)
      : []
    if (matches.length !== 1 || !/^[0-9a-f]{64}$/.test(matches[0]?.sha256 ?? "")) {
      throw new Error(
        `${relative(manifestPath)} must contain exactly one pinned ${upstreamPath} -> ${vendoredPath} entry`
      )
    }
    return matches[0]
  }
...
`
  ...
  function generateProtocolRegistries(prefix, descriptors, options = {}) {
    ...
    const codecByType = objectLiteral(
      descriptors.map((descriptor) => [descriptor.type, `Generated.${descriptor.type}`])
    )
...
${name}ClassFields${annotations}
  ...
  function generateSchemaRegistry() {
    ...
    return `// MCP $defs codec registry generated from schema.json. Do not edit.
...
${names.map((name) => `  ${json(name)}: ${name}`).join(",\n")}
  ...
  function generatedBanner(sourceName) {
    return `/**
 * Generated from ${sourceName} for MCP ${protocolVersion}.
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  NodeRuntime.runMain(runScript("generate-mcp", runGenerateMcp))
}

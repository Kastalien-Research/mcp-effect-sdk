...
const forbiddenPlatformNames = /\b(?:Window|Document|HTMLElement|MessageEvent)\b|\blib=["']dom(?:\.iterable)?["']/i
...
while (pending.length > 0) {
  ...
  assert.equal(existsSync(absolute), true, `missing core declaration: ${relative}`)
  const source = readFileSync(absolute, "utf8")
  assert.doesNotMatch(source, forbiddenPlatformNames, `${relative} must be DOM-free`)
  ...
  for (const match of source.matchAll(declarationImport)) {
    ...
    pending.push(resolved.endsWith(".js") ? `${resolved.slice(0, -3)}.d.ts` : `${resolved}.d.ts`)
  }
}

console.log(`WP5B core declaration graph is DOM/Node-free (${visited.size} files).`)

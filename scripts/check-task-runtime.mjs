  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      yield* server.addTool({
        ...
        handle: () => Deferred.await(release).pipe(Effect.as(result("finished")))
      })
      ...
      assert.equal(registeredPayload.content[0].text, `registered task support ${registeredCreated.task.taskId}`)
    })
  )
)

const cancelRuntime = await Effect.runPromise(McpTasks.McpTasks.make())
const cancellable = await Effect.runPromise(
  cancelRuntime.start({
    ttl: 5000,
    effect: Effect.never
  })
...
const throwingFactory = await Effect.runPromise(
  throwingFactoryRuntime.start({
    effect: () => {
...
const failedFactoryTask = await Effect.runPromise(throwingFactoryRuntime.get({ taskId: throwingFactory.task.taskId }))
...
await assertFails(ttlRuntime.get({ taskId: expiring.task.taskId }), "ttl-expired tasks are no longer retrievable")

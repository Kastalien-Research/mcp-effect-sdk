...
const acknowledgement = (id, notifications = {}) =>
  notification("notifications/subscriptions/acknowledged", { notifications, _meta: subscriptionMeta(id) })
const subscriptionSuccess = (id, subscriptionId = id, result = {}) =>
  success(id, {
    resultType: "complete",
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const failLocally = (id) =>
          Effect.gen(function* () {
            const active = yield* collect(
              client,
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const fiber = yield* collect(
          client,
          request("sub", "subscriptions/listen", {
        ...
        assert.deepEqual(
          frames.slice(0, 2).map((frame) => frame.notification),
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const progressed = yield* collect(
          client,
          request("progress-owner", "tools/list", {
            _meta: { progressToken: "progress-token" }
          })
        ).pipe(Effect.forkScoped)
        const subscription = yield* collect(
          client,
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const active = yield* collect(
          client,
          request("invalid-cancel-owner", "subscriptions/listen", {
            notifications: {}
          })
        ).pipe(Effect.forkScoped)
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.deepEqual(
          remaining.slice(0, 16).map((frame) => frame.notification.params.sequence),
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.equal(handled, 1)
        assert.deepEqual(
          sent.slice(1).map((message) => message.error.code),
          [-32601, -32602, -32602]
        )
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.deepEqual(
          writes.map((message) => message.result.sequence),
          [1]
        )
    ...
    Effect.scoped(
      Effect.gen(function* () {
        for (const failureKind of cases) {
          ...
          const published = yield* Queue.take(server.failures).pipe(Effect.timeoutOption("250 millis"))
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.deepEqual(
          Array.from(yield* Fiber.join(clientFiber)).map((frame) => frame._tag),
          ["Notification", "Success"]
        )
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.deepEqual(
          sent[0].result.tools.map((tool) => tool.name),
          ["visible"]
        )
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* server.accept(request("1", "tools/list", validParams()))
        yield* Queue.takeN(handlerEvents, 2)
        const invalidCancellation = yield* server
          .accept(
            notification("notifications/cancelled", {
              requestId: 1,
              reason: 123

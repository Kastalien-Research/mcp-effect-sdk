...
const makeProgressServer = (observeContext) =>
  McpServer.make({
...
test("stable sendProgress derives exact empty-string and zero tokens from a distinct request facade", async () => {
  ...
  assert.equal(
    observedKeys.every((keys) => !keys.includes("notificationSink")),
    true
  )
})
...
test("missing and hostile facade tokens plus invalid updates fail typed before the owned sink", async (t) => {
  ...
  await t.test("hostile Option accessor is never invoked", async () => {
    ...
    const result = await Effect.runPromise(
      McpServer.sendProgress({ progress: 1 }).pipe(
...
test("server public progress inspection is exact-own-data and contains hostile traps", async (t) => {
  ...
  const cases = [
    ...
    {
      name: "accessor update",
      context: { progressToken: Option.some("owned"), reportProgress: () => Effect.void },
      update: accessorUpdate
    },
    {
      name: "inherited token value",
      context: { progressToken: inheritedToken, reportProgress: () => Effect.void },
      update: { progress: 1 }
    },
    {
      ...
      update: new Proxy(
        ...
        {
          ownKeys() {
            throw new Error("ownKeys escaped")
          }
        }
      )
    },
    ...
    },
    {
      ...
      update: new Proxy(
        ...
        {
          getPrototypeOf() {
            throw new Error("prototype escaped")
          }
        }
      )
    },
    {
      ...
      context: {
        progressToken: new Proxy(
          { _tag: "Some", value: "owned" },
          {
            getOwnPropertyDescriptor() {
              throw new Error("token descriptor escaped")
            }
          }
        ),
    ...
    },
    {
      ...
      context: new Proxy(
        { progressToken: Option.some("owned"), reportProgress: () => Effect.void },
        {
          get() {
            throw new Error("context get escaped")
          }
        }
      ),
      update: { progress: 1 }
  ...
  for (const testCase of cases)
    await t.test(testCase.name, async () => {
      ...
      const context =
        base instanceof Object && !Object.hasOwn(base, "reportProgress")
          ? base
          : new Proxy(base, {

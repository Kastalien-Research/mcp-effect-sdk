...
const complete = (method, overrides = {}) => {
  ...
  switch (method) {
    ...
    case "tools/list":
      return { ...common, tools: [], ...overrides }
    case "resources/list":
      return { ...common, resources: [], ...overrides }
    case "resources/templates/list":
      return { ...common, resourceTemplates: [], ...overrides }
    case "resources/read":
      return { ...common, contents: [], ...overrides }
    case "prompts/list":
      return { ...common, prompts: [], ...overrides }
    case "tools/call":
      return { resultType: "complete", content: [], ...overrides }
    default:
      return { resultType: "complete", completion: { values: [] }, ...overrides }
  }
}
...
const makeTransport = (options = {}) => {
  ...
  const transport = {
    request: (request) => {
      calls.push(request)
      const result =
        options.result?.(request, calls.length) ?? complete(request.method, options.overrides?.[request.method])
...
const scopedClient = (options, use) =>
  Effect.runPromise(Effect.scoped(ClientApi.make(options).pipe(Effect.flatMap(use))))
...
test("all six cacheable methods hit positive-TTL entries while zero TTL misses", async () => {
  ...
  await scopedClient({ transport: probe.transport, cache, cacheNamespace: "all-methods" }, (client) =>
    Effect.gen(function* () {
...
test("public entries share explicitly while private entries default to bypass", async () => {
  ...
  await scopedClient(
    { transport: privateFirst.transport, cache: privateCache, cacheNamespace: "private" },
    () => Effect.void
  )
  await scopedClient(
    { transport: privateSecond.transport, cache: privateCache, cacheNamespace: "private" },
    () => Effect.void
  )
...
test("list-change and resource-updated notifications invalidate before exposure", async () => {
  ...
  const probe = makeTransport({
    frames: (request, result) =>
      notify && request.method === "tools/call"
        ? [
            {
              _tag: "Notification",
              notification: {
  ...
  await scopedClient({ transport: probe.transport, cache, cacheNamespace: "invalidate" }, (client) =>
    Effect.gen(function* () {
      yield* client.listTools()
      yield* client.listTools()
      yield* client.readResource({ uri: "test://one" })
...
test("memory cache capacity is deterministic and corrupt values invalidate to a miss", async () => {
  ...
  const probe = makeTransport()
  await scopedClient({ transport: probe.transport, cache, cacheNamespace: "capacity" }, (client) =>
    Effect.gen(function* () {
      yield* client.listTools()
      yield* client.listResources()
      yield* client.listTools()
...
test("freshness equality is stale and overflow expiration saturates safely", async () => {
  ...
  await scopedClient({ transport: probe.transport, cache, cacheNamespace: "freshness" }, (client) =>
    Effect.gen(function* () {
...
test("cache stores immutable exact wire JSON and re-decodes every hit", async () => {
  ...
  const probe = makeTransport({
    result: (request) => (request.method === "tools/list" ? result : complete(request.method))
  })
  await scopedClient({ transport: probe.transport, cache, cacheNamespace: "immutable" }, (client) =>
    Effect.gen(function* () {
...
test("protocol errors and invalid cacheable results are never stored", async () => {
  ...
  const probe = makeTransport({
    result: (request) =>
      request.method === "tools/list" ? { resultType: "input_required", inputRequests: {} } : complete(request.method)
  })
  const outcome = await Effect.runPromise(
    Effect.scoped(
...
test("authorization provider is exact, never inspects principal-like extras, and failures are typed", async () => {
  ...
  Object.defineProperty(tagged, "principal", {
    get() {
      getterCalls += 1
      throw new Error("principal-read")
    }
  })
  await scopedClient(
    {
...
test("set and invalidate infrastructure failures own the request", async () => {
  const methods = [
    {
      get: () => Effect.succeed({ _tag: "None" }),
      set: () => Effect.fail("set-failure"),
      invalidate: () => Effect.void
    },
    {
      get: () =>
        Effect.succeed({ _tag: "Some", value: { result: {}, receivedAt: 0, expiresAt: 0, cacheScope: "public" } }),
      set: () => Effect.void,
      invalidate: () => Effect.fail("invalidate-failure")
    }
  ]
...
test("invalidation epochs prevent an in-flight stale response from repopulating", async () => {
        ...
        return Stream.unwrap(
          Deferred.await(gate).pipe(
            Effect.as(
              Stream.succeed({
                _tag: "Success",
                response: { _tag: "SuccessResponse", jsonrpc: "2.0", id: request.id, result: complete("tools/list") }
              })
            )
          )
        )
      ...
      const frames =
        request.method === "tools/call"
          ? [
              ...
              {
                _tag: "Success",
                response: { _tag: "SuccessResponse", jsonrpc: "2.0", id: request.id, result: complete("tools/call") }
              }
            ]
          : [
              {
  ...
  await scopedClient({ transport, cache }, (client) =>
    Effect.gen(function* () {
      const before = sets
          ...
          : [
              {
                ...
                response: {
                  ...
                  jsonrpc: "2.0",
                  id: request.id,
                  result: complete(request.method, {
...
test("invalidation during a delayed cache set removes the late stale write", async () => {
  ...
  const cache = {
    get: (key) =>
      Effect.succeed(
        key.method === "tools/list" && stored !== undefined ? { _tag: "Some", value: stored } : { _tag: "None" }
      ),
    set: (key, entry) =>
      key.method === "tools/list" && blockFirstToolSet
        ? Effect.gen(function* () {

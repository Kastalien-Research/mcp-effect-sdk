...
const respondingTransport = (requests = []) => ({
  request: (request) => {
    ...
    return Stream.succeed(
      success(request, request.method === "server/discover" ? discoverResult() : completeResult(request.method))
    )
  }
})

const runClient = (options, use) =>
  Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
...
test("client info is exact-validated once and its canonical snapshot is attached to every request", async () => {
  ...
  await runClient({ transport: respondingTransport(requests), clientInfo }, (client) =>
    Effect.gen(function* () {
...
test("constructor properties are descriptor-snapshotted once and provider replacement after make has no effect", async () => {
  ...
  await runClient(options, (client) =>
    Effect.gen(function* () {
      target.capabilities = () =>
        Effect.succeed({
    ...
    await t.test(property, async () => {
      ...
      Object.defineProperty(options, property, {
        ...
        get() {
          getterCalls += 1
          if (getterCalls === 1) {
            return property === "clientInfo" ? { name: "temporal-client", version: "1" } : () => Effect.succeed({})
          }
...
test("provider environments are captured during make and are not required by returned methods", async () => {
  ...
  assert.deepEqual(requests[1].params._meta[CLIENT_CAPABILITIES_KEY].experimental, {
    "com.example/captured-profile": { name: "captured", requestId: 2 }
  })
  assert.deepEqual(requests[1].params._meta[CLIENT_CAPABILITIES_KEY].extensions, {
...
test("Deferred-controlled concurrent providers preserve interleaving isolation", async () => {
  ...
  for (const request of ordinary) {
    ...
    assert.equal(request.params._meta[CLIENT_CAPABILITIES_KEY].extensions["com.example/profile"].method, request.method)
  }
  assert.notEqual(ordinary[0].params._meta[CLIENT_CAPABILITIES_KEY], ordinary[1].params._meta[CLIENT_CAPABILITIES_KEY])
})
...
test("extension authority grammar and JSONObject settings are shared by client construction", async (t) => {
  ...
  assert.deepEqual(requests[1].params._meta[CLIENT_CAPABILITIES_KEY].extensions, valid)
})
...
test("mutating a provider result after completion cannot alter the request snapshot", async () => {
  ...
  assert.deepEqual(ordinaryRequest.params._meta[CLIENT_CAPABILITIES_KEY].experimental, {
    "com.example/snapshot": { value: 1 }
  })
})

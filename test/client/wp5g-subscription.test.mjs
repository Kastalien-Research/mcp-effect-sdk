...
const makeTransport = (subscription) => ({
  request: (request) =>
    request.method === "server/discover" ? Stream.succeed(success(request, discoverResult())) : subscription(request)
})

const makeClient = (transport) =>
  McpClient.make({
    transport,
    clientInfo: { name: "wp5g-client", version: "1.0.0" }
  })

const runScoped = (effect, timeout = "2 seconds") =>
  Effect.runPromise(Effect.scoped(effect).pipe(Effect.timeout(timeout)))
  ...
  await runScoped(
    Effect.gen(function* () {
      ...
      const [toolEvents, resourceEvents] = yield* Effect.all(
        [tools.notifications.pipe(Stream.runCollect), resources.notifications.pipe(Stream.runCollect)],
        { concurrency: "unbounded" }
      )
      assert.deepEqual(
...
test("a generated terminal is graceful and ends the notification stream", async () => {
  const transport = makeTransport((request) => Stream.make(acknowledgement(request), graceful(request)))
...
test("a post-terminal frame is ProtocolError unless caller close already won", async () => {
  ...
  const postTerminal = makeTransport(() =>
    Stream.make(acknowledgement(request), graceful(request), changed(request, "notifications/tools/list_changed"))
  )
  await runScoped(
    Effect.gen(function* () {
...
test("terminal-pending teardown failure is graceful", async () => {
  const transport = makeTransport((request) =>
    Stream.make(acknowledgement(request), graceful(request)).pipe(
      Stream.concat(Stream.fail(new TransportError({ message: "teardown failed" })))
    )
  )
...
test("hostile transport failures and hostile cause data always settle", async () => {
  ...
  const hostileCause = new Proxy(
    ...
    {
      get: () => {
        throw new Error("cause get trap")
      },
      has: () => {
        throw new Error("cause has trap")
      }
    }
  )

  const preAckObserved = await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        const client = yield* makeClient(makeTransport(() => Stream.fail(hostileFailure)))
        return yield* client.subscriptionsListen().pipe(Effect.exit, Effect.timeoutOption("100 millis"))
      })
    )
  )
...
test("pure transport interruption is Abrupt while notifications terminate by interruption", async () => {
  const transport = makeTransport((request) =>
    Stream.make(acknowledgement(request)).pipe(Stream.concat(Stream.fromEffect(Effect.interrupt)))
  )
...
test("detected overflow, protocol, and dispatch failures beat stream-finalizer close", async () => {
  ...
  await runRace({
    ...
    frames: (request, release) =>
      Stream.make(acknowledgement(request, { toolsListChanged: true })).pipe(
        Stream.concat(
          Stream.fromEffect(
            Deferred.await(release).pipe(Effect.as(changed(request, "notifications/tools/list_changed")))
          )

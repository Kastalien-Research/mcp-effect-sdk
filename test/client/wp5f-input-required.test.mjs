...
const scopedClient = (transport, inputRequired, capabilities) =>
  Effect.scoped(
    Client.make({
      transport,
...
test("stable InputRequiredPolicy exposes automatic and manual construction", () => {
  ...
  let getterReads = 0
  assert.throws(() =>
    Client.InputRequiredPolicy.automatic(
      Object.defineProperty({}, "maxRounds", {
        enumerable: true,
        get: () => {
          getterReads++
          return 1
...
test("manual mode sends once and permits an exact caller-owned continuation", async () => {
  ...
  const client = await Effect.runPromise(
    scopedClient(transport, { mode: "manual" }, () => Effect.succeed({ elicitation: { form: {} } }))
  )
...
test("automatic MRTR preserves exact keys/state, bounds concurrency, supports resume and reentrancy", async () => {
  ...
  for (const key of keys)
    Object.defineProperty(inputRequests, key, {
      enumerable: true,
  ...
  await t.test("automatic policy owns advertised input capabilities", async () => {
    ...
    const transport = {
      request: (request) => {
        ...
        return Stream.succeed(
          success(request, request.method === "server/discover" ? discover : complete[request.method])
        )
      }
    }
    const outcome = await Effect.runPromise(

...
const grantHandle = (client, value = "grant-wp6d") => Schema.decodeUnknownSync(client.AuthorizationGrantHandle)(value)
...
const callback = (client, parameters, overrides = {}) =>
  Schema.decodeUnknownSync(client.AuthorizationCallbackInput)({
    ...
    service: {
      ...
      readCredential: (handle) =>
        Effect.sync(() => {
          ...
          return (
            options.credentials?.get(handle) ??
            options.credential ?? {
              issuer: "https://issuer.example",
              clientId: "wp6d-client",
...
const providePorts = (effect, client, http, store) =>
  effect.pipe(
...
test("authorization-code exchange sends exact code, verifier, redirect, and resource before persisting audience-validated opaque tokens", async () => {
  ...
  assert.deepEqual(events, ["store:readCredential", "http:token", "validate:audience", "store:saveGrant"])
...
test("authorization code exchange remains bound to the exact start credential among same-issuer clients", async () => {
  ...
  assert.deepEqual(
    store.calls.map(([operation, value]) => [operation, value]),
    [
  ...
  assert.equal(
    store.calls.some(([operation]) => operation === "findCredential"),
    false
  )
})
...
test("refresh is issuer-partitioned, sends the RFC 8707 resource, and preserves an unrotated refresh token", async () => {
  const {
    client,
    token: { refreshAuthorizationGrant }
  } = await loadWp6d()
  const issuer = "https://issuer.example"
  ...
  assert.deepEqual(
    ...
    ["readGrant", "findCredential", "readCredential", "saveGrant"]
  )
...
test("token expiry uses the Effect Clock when receivedAt is absent and accepts immediate zero-second expiry", async () => {
  ...
  assert.deepEqual(
    outcomes.map(({ fixture, result, store }) => ({
      name: fixture.name,
      result: result._tag,
...
test("Bearer token types are case-insensitive and canonical while non-Bearer responses and grants fail before downstream work", async () => {
  const {
    client,
    token: { exchangeAuthorizationCode, refreshAuthorizationGrant }
  } = await loadWp6d()

  const mixedCaseHttp = makeHttp(() =>
    Effect.succeed(
      jsonResponse({
        access_token: tokenSecret,
        token_type: "bEaReR"
...
test("callback state, redirect, response iss, and denial failures prevent every token request", async () => {
  const {
    client,
    token: { exchangeAuthorizationCallback }
  } = await loadWp6d()
  const issuer = "https://issuer.example"
  const cases = [
    {
...
test("token endpoint failures are typed and never retain code, verifier, token, refresh token, or response bodies", async () => {
  ...
  for (const fixture of cases) {
    ...
    assert.deepEqual(store.saved, [], fixture.name)
    assertSecretsAbsent(error, [codeSecret, verifierSecret, tokenSecret, refreshSecret, responseSecret])
  }
})
...
test("rehydrated transaction and completed authorization without client identity reject same-issuer credential substitution before token ports", async () => {
  const {
    client,
    token: { exchangeAuthorizationCallback, exchangeAuthorizationCode }
  } = await loadWp6d()
  const issuer = "https://issuer.example"
  ...
  const fixtures = [
    {
      ...
      run: (http, store, validateAudience) =>
        exchangeAuthorizationCallback({
          callback: callback(
            client,
            new URLSearchParams({
...
test("token endpoint authentication selects none, post, or Basic and rejects unsupported or inconsistent methods before HTTP", async () => {
  ...
  for (const fixture of fixtures) {
    const credential = {
      ...
      ...(fixture.clientSecret === undefined ? {} : { clientSecret: Redacted.make(fixture.clientSecret) })
    }
    ...
    const authorizationValue = authorizationHeader === undefined ? undefined : Redacted.value(authorizationHeader[1])
    const basicMatches =
      authorizationValue?.startsWith("Basic ") === true &&
      Buffer.from(authorizationValue.slice(6), "base64").toString("utf8") === `wp6d-client:${clientAuthSecret}`
    outcomes.push({
      fixture,
      result,
      store,
      ...
      placement: {
        ...
        secretMatches:
          fixture.method === "client_secret_basic" ? basicMatches : body?.get("client_secret") === clientAuthSecret
      }
    })
  }

  assert.deepEqual(
    outcomes.map(({ fixture, result, store, http, audienceCalls, placement }) =>
      fixture.succeeds
        ? {
            name: fixture.name,
            result: result._tag,
        ...
        providePorts(
          exchangeAuthorizationCode({
            ...
            authorizationServerMetadata: metadata(
              ...
              issuer,
              undefined,
              fixture.advertised === undefined ? {} : { token_endpoint_auth_methods_supported: fixture.advertised }
            ),
            validateAudience: ({ resource }) => Effect.succeed(Object.freeze([resource]))
          }),
          client,
      ...
      method:
        authorizationHeader === undefined
          ? body?.get("client_secret") === clientAuthSecret
            ? "post"
            : "none"
          : basicMatches && Redacted.isRedacted(authorizationHeader[1])
            ? "basic"
            : "invalid"
    })
  }

...
const decode = (chunks, options) =>
  StdioTransport.decode(Stream.fromIterable(chunks), options).pipe(Stream.runCollect, Effect.map(Array.from))

test("stdio byte framing preserves split UTF-8, multi-message chunks, CRLF, and escaped newlines", async () => {
  const first = JSON.stringify(
    wire({ ...request("one", "fixture/method"), params: { label: "snowman ☃ and escaped\\nline" } })
  )
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const hanging = yield* client
          .request(request("cancel-me", "test/hang"))
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const client = yield* StdioClientTransport.make({
          ...
          stderrSink: (chunk) =>
            new TextDecoder().decode(chunk).includes("stdin-closed")
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        }).pipe(Effect.provideService(McpServer.McpServer, service), Effect.forkScoped)
        yield* Queue.offer(
          input,
          bytes(
            `${JSON.stringify(
      ...
      Effect.gen(function* () {
        ...
        for (const id of [1, "1"]) {
          ...
          assert.equal(
            ...
            true,
            `subscription was not acknowledged; opened=${JSON.stringify(opened)}`
          )
          assert.equal(acknowledged.value.method, "notifications/subscriptions/acknowledged")
          assert.strictEqual(acknowledged.value.params._meta["io.modelcontextprotocol/subscriptionId"], id)
          yield* expectNone()
        }
        assert.deepEqual(opened, [1, "1"])

        yield* service.publish({ tag: "notifications/tools/list_changed", payload: {} })
        const published = [yield* take(), yield* take()].map(
          (value) => value.value.params._meta["io.modelcontextprotocol/subscriptionId"]
        )
        ...
        assert.strictEqual(remaining.value.params._meta["io.modelcontextprotocol/subscriptionId"], "1")
        yield* expectNone()
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        Object.defineProperty(accessor, "value", {
          enumerable: true,
          get: () => {
            throw new Error("must not invoke result accessors")
          }
        })
...
test("default stderr diagnostics preserve the primary stage and survive a broken diagnostic pipe", async () => {
  ...
  assert.equal(working.stderr, "ready\nmcp-effect-sdk: stdio server transport terminated at Write\n")
...
test("active stdio sources reject legacy framing and unsafe event bridges", () => {
  ...
  assert.doesNotMatch(
    client,
    /makeCompatibilityProtocol|readonly notifications:|sendNotification|readonly cancel:|readonly closed:/
  )
})
...
test("process stream bridges use bounded event queues ahead of byte framing", () => {
  for (const relative of ["src/transport/StdioClientTransport.ts", "src/transport/StdioServerTransport.ts"]) {

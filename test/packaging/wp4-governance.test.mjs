...
test("package scripts expose cumulative WP4 HTTP and transport gates", () => {
  ...
  assert.equal(scripts["test:http"], "pnpm run build && node scripts/run-http-suite.mjs")
  assert.equal(scripts["test:transports"], "pnpm run build && node scripts/run-transports-suite.mjs")
  assert.equal(scripts["check:ts-sdk-parity"], "node scripts/check-ts-sdk-parity.mjs")
  for (const relative of ["scripts/run-http-suite.mjs", "scripts/run-transports-suite.mjs"])
...
test("parity is self-contained against the final specification and validates an explicit implementation/deferred ledger", () => {
  ...
  assert.equal(
    ledger.items.slice(2).every(({ status }) => status === "deferred"),
    true
  )

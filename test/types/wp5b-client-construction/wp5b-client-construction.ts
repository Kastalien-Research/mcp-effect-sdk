...
const construction: Effect.Effect<McpClient.McpClient, McpClientError, Scope.Scope | Profile> = McpClient.make({
  transport,
  clientInfo: {
...
const methodHasNoProviderRequirements: Effect.Effect<unknown, McpClientError, never> = client.listTools()

const optionalIdentity: Effect.Effect<McpClient.McpClient, McpClientError, Scope.Scope> = McpClient.make({ transport })

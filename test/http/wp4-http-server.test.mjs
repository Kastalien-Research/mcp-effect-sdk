...
const explicitServerLayer = (appLayer, combined) =>
  appLayer.pipe(Layer.provideMerge(McpServer.layer(serverConstructionOptions(combined))))
...
const toWebHandler = (appLayer, combined) => {
  ...
  try {
    ...
    const web = StreamableHttpServerTransport.toWebHandler(server, transportOptions(combined))
    return {
      handler: web.handler,
      dispose: async () => {
        await web.dispose()
...
const withServer = (serverOptions, run) => withServerLayer(Layer.empty, serverOptions, run)
...
const assertSelectedProtocol = (response) => {
  assert.equal(response.headers.get(McpModern.MCP_PROTOCOL_VERSION_HEADER), protocolVersion)
}

test("Effect Platform routes every method through the exact modern handler", async () => {
  await withEffectPlatform(
    options({
    ...
    async (handler) => {
      ...
      assert.equal(frame.value.result._meta["io.modelcontextprotocol/serverInfo"].name, "wp4-http-server")
...
const rpcPost = (input) => {
  const { id, method, params, nameHeader, headers = {} } = input
  const protocolHeader = Object.hasOwn(input, "protocolHeader") ? input.protocolHeader : protocolVersion
  const methodHeader = Object.hasOwn(input, "methodHeader") ? input.methodHeader : method
...
const emptyCallResult = (structuredContent = {}) =>
...
const validationProbeLayer = (counters) =>
  Layer.effectDiscard(
    Effect.gen(function* () {
...
const streamToolLayer = (name, handler) =>
  Layer.effectDiscard(
    Effect.gen(function* () {
      const server = yield* McpServer.McpServer
      yield* server.addTool({
...
const callToolRequest = (id, name, argumentsValue = {}) =>
  rpcPost({
    id,
    method: "tools/call",
    nameHeader: name,
...
const subscriptionRequest = (id, notifications, overrides = {}) =>
  rpcPost({
    id,
...
test("raw Web routing matches the configured pathname exactly and permits queries", async () => {
  ...
  await withServer(options(), async (handler) => {
    ...
    assert.deepEqual(
      await Promise.all(
        wrongResponses.map(async (response, index) => ({
          status: response.status,
...
test("invalid maxBodyBytes values are rejected before any request body can be accessed", async () => {
  ...
  for (const maxBodyBytes of [0, -1, 1.5, Number.MAX_SAFE_INTEGER + 1, Infinity, Number.NaN]) {
    let web
    try {
      web = toWebHandler(Layer.empty, options({ maxBodyBytes }))
...
test("required protocol and method headers fail before handlers or subscriptions", async () => {
  ...
  await withServerLayer(validationProbeLayer(counters), options(), async (handler) => {
    ...
    const subscriptionResponse = await handler(
      rpcPost({
    ...
    const subscriptionObservation = {
      ...
      protocolVersion: subscriptionResponse.headers.get(McpModern.MCP_PROTOCOL_VERSION_HEADER)
    }
...
test("required Mcp-Name headers fail for every generated name source before registry access", async () => {
  ...
  await withServerLayer(validationProbeLayer(counters), options(), async (handler) => {
    ...
    assert.deepEqual({ observations, counters }, { observations: expected, counters: freshCounters() })
  })
})
        ...
        await errorObservation(
          await handler(
            rpcPost({
              ...
              headers: entry.customHeader === undefined ? {} : { "Mcp-Param-Region": entry.customHeader }
            })
          )
        )
      )
...
test("outbound SSE validates known notifications while preserving extensions", async () => {
  ...
  const app = Layer.mergeAll(
    streamToolLayer("malformed-known-notification", () =>
      McpDispatcher.McpRequestContext.pipe(
    ...
    streamToolLayer("extension-notification", () =>
      McpDispatcher.McpRequestContext.pipe(
        Effect.flatMap((context) =>
          context.notificationSink({
            _tag: "Notification",
  ...
  await withServerLayer(app, options({ enableJsonResponse: undefined }), async (handler) => {
    const malformed = await handler(callToolRequest("malformed-known-notification", "malformed-known-notification"))
    const reader = malformed.body.getReader()
    const malformedRead = await promptOutcome(
      reader.read().then(
        () => ({ _tag: "Resolved" }),
    ...
    assert.equal((await promptOutcome(Effect.runPromise(Deferred.await(malformedCompleted)), 500))._tag, "Response")

    const extension = await handler(callToolRequest("extension-notification", "extension-notification"))
...
test("maxPendingFrames is validated and bounds unread SSE producers", async () => {
  ...
  for (const maxPendingFrames of [0, -1, 1.5, Number.MAX_SAFE_INTEGER + 1]) {
    try {
      const web = toWebHandler(Layer.empty, options({ maxPendingFrames }))
  ...
  for (const configuration of [
    ...
    const produced = []
    const toolName = configuration.maxPendingFrames === undefined ? "default-bound" : "small-bound"
    const app = streamToolLayer(toolName, () =>
      McpDispatcher.McpRequestContext.pipe(
        Effect.flatMap((context) =>
          Effect.forEach(
            ...
            (index) =>
              context
                .notificationSink(progressNotification(`${toolName}-${index}`))
                .pipe(Effect.tap(() => Effect.sync(() => produced.push(index)))),
            { discard: true }
          )
        ),
    ...
    await withServerLayer(
      ...
      options({
        ...
        ...(configuration.maxPendingFrames === undefined ? {} : { maxPendingFrames: configuration.maxPendingFrames })
      }),
      async (handler) => {
...
test("concurrent numeric and string IDs isolate ordinary SSE responses", async () => {
  ...
  await withServerLayer(app, options({ enableJsonResponse: undefined }), async (handler) => {
    ...
    const numericResponse = numericPrompt._tag === "Response" ? numericPrompt.value : await numericPending
    const stringResponse = stringPrompt._tag === "Response" ? stringPrompt.value : await stringPending
    const [numericText, stringText] = await Promise.all([numericResponse.text(), stringResponse.text()])
    const numericMessages = numericText.endsWith("\n\n") ? parseSseMessages(numericText) : { invalidBody: numericText }
    const stringMessages = stringText.endsWith("\n\n") ? parseSseMessages(stringText) : { invalidBody: stringText }
    assert.deepEqual(
      {
...
test("extension notifications are typed, authorized, and isolated from core cancellation", async () => {
  ...
  const notificationPost = (method, marker) =>
    new Request("http://localhost/mcp", {
      method: "POST",
      headers: {
        "content-type": "application/json",
  ...
  await withServer(
    options({
      acceptNotification: (notification, context) =>
        Effect.sync(() => {
    ...
    async (handler) => {
      ...
      assert.deepEqual(
        calls.map(({ notification, context }) => ({
          notification,
          authorizationPrincipal: context.authorizationPrincipal
        })),
        [
...
test("subscription acknowledgement wins registration races and filters every later frame", async () => {
  ...
  await withServerLayer(probe.layer, options({ enableJsonResponse: undefined }), async (handler) => {
    ...
    const laterPublish = await promptOutcome(
      Effect.runPromise(
        probe.service().publish({
          tag: "notifications/tools/list_changed",
...
test("independent numeric and string subscriptions never cross filtered streams", async () => {
  ...
  await withServerLayer(probe.layer, options({ enableJsonResponse: undefined }), async (handler) => {
    ...
    if (acknowledgements.some((entry) => entry._tag !== "Message")) {
      await Promise.all([numeric.reader.cancel().catch(() => undefined), string.reader.cancel().catch(() => undefined)])
      assert.deepEqual(
    ...
    assert.deepEqual(
      {
        acknowledgements: acknowledgements.map(
          (entry) => entry.value?.params?._meta?.["io.modelcontextprotocol/subscriptionId"]
        ),
        numericFrame,
        stringFrame,
        surviving,
...
test("disposing the Web handler closes active subscription streams idempotently", async () => {
  const probe = subscriptionProbe()
  const web = toWebHandler(probe.layer, options({ enableJsonResponse: undefined }))
  let cursor
  try {
    const response = await web.handler(
      subscriptionRequest("dispose-listen", {
        toolsListChanged: true
      })
    )
    ...
    await web.dispose()
    const closed = await waitUntil(() => probe.closed.length === 1)
    const later = await promptOutcome(
      Effect.runPromise(
        probe.service().publish({
          tag: "notifications/tools/list_changed",
...
test(
  "real Node HTTP bridge streams subscriptions incrementally and cleans abrupt sockets",
  {
    timeout: 5_000
  },
  async () => {
    const probe = subscriptionProbe()
    const web = toWebHandler(probe.layer, options({ enableJsonResponse: true }))
...
test("extension notifications complete generic preflight before hook side effects", async () => {
  ...
  await withServer(
    options({
      ...
      acceptNotification: (_message, context) =>
        Effect.sync(() => {
          calls.push(context)
        })
    }),
...
test("parsed bodies and early oversized uploads honor the raw Content-Length bound", async () => {
  ...
  await withServerLayer(app, options({ maxBodyBytes: 64 }), async (handler) => {
    ...
    const parsedResponse = await handler(
      new Request("http://localhost/mcp", {
        method: "POST",
        headers: {
          "content-type": "application/json",
...
test("parsed body trust fields reject accessors without invocation and release raw input", async () => {
  ...
  for (const field of ["parsedBody", "parsedBodyByteLength"]) {
    for (const behavior of ["return", "throw"]) {
      ...
      const handleOptions =
        field === "parsedBody" ? { parsedBodyByteLength: raw.byteLength } : { parsedBody: requestBody() }
      ...
      await withServer(options({ maxBodyBytes: 1024 }), async (handler) => {
        ...
        const publicBody =
          outcome._tag === "Response" && outcome.value._tag === "Resolved"
  ...
  assert.deepEqual(
    observations,
    ["parsedBody", "parsedBodyByteLength"].flatMap((field) =>
      ["return", "throw"].map((behavior) => ({
        field,
        behavior,
        outcome: "Response",
        result: "Resolved",
...
test("parsed body descriptor traps fail safe and release raw input", async () => {
  const raw = new TextEncoder().encode(JSON.stringify(requestBody()))
  const observations = []
  for (const trappedField of ["parsedBody", "parsedBodyByteLength"]) {
    const trapCause = new Error(`synthetic-${trappedField}-descriptor-secret`)
    ...
    const handleOptions = new Proxy(target, {
      getOwnPropertyDescriptor(value, property) {
        if (property === trappedField) {
          descriptorTraps++
          throw trapCause
        }
      ...
      get() {
        propertyGets++
        throw new Error("property-get-secret")
      }
    })
    ...
    await withServer(options({ maxBodyBytes: 1024 }), async (handler) => {
      ...
      observations.push({
        ...
        result: outcome._tag === "Response" ? outcome.value._tag : undefined,
        status:
          outcome._tag === "Response" && outcome.value._tag === "Resolved" ? outcome.value.response.status : undefined,
        publicBody,
        leaked: publicBody?.includes(trapCause.message) ?? false,
        descriptorTraps,
        propertyGets,
        cancelled,
        locked: body.locked
  ...
  assert.deepEqual(
    observations,
    ["parsedBody", "parsedBodyByteLength"].map((trappedField) => ({
      trappedField,
      outcome: "Response",
      result: "Resolved",
      status: 400,
  ...
  const consumed = post({ body: new TextDecoder().decode(raw) })
  await consumed.text()
  await withServer(options({ maxBodyBytes: 1024 }), async (handler) => {
    const response = await handler(consumed, {
      parsedBody: requestBody(),
      parsedBodyByteLength: raw.byteLength
    })
    assert.equal(response.status, 200)
...
test("oversize cleanup diagnostics never delay 413 or handler disposal", async () => {
  ...
  for (const sinkMode of ["complete", "fail", "defect", "interrupt", "never"]) {
    ...
    try {
      ...
      assert.equal((await promptOutcome(Effect.runPromise(Deferred.await(sinkEntered)), 500))._tag, "Response")
      const handlerOutcome = await promptOutcome(pending, 150)
      const publicBody =
        handlerOutcome._tag === "Response" && handlerOutcome.value._tag === "Resolved"
      ...
      observations.push({
        ...
        result: handlerOutcome._tag === "Response" ? handlerOutcome.value._tag : undefined,
        status:
          handlerOutcome._tag === "Response" && handlerOutcome.value._tag === "Resolved"
  ...
  assert.deepEqual(
    observations,
    ["complete", "fail", "defect", "interrupt", "never"].map((sinkMode) => ({
      sinkMode,
      handler: "Response",
      result: "Resolved",
      status: 413,
...
test("Effect-native scoped handle offers cleanup diagnostics before scope closure", async () => {
  ...
  for (let index = 0; index < repetitions; index++) {
    ...
    const responseOutcome = await promptOutcome(
      Effect.runPromise(
        Effect.scoped(
  ...
  const throwingOutcome = await promptOutcome(
    Effect.runPromise(
      Effect.scoped(
        StreamableHttpServerTransport.handle(
          new Request("http://localhost/mcp", {
...
test("concurrent caller disposal never waits for cleanup diagnostic start", async () => {
  ...
  for (const owner of ["scope", "web-adapter"]) {
    ...
    if (owner === "scope") {
      ...
      const web = toWebHandler(Layer.empty, transportOptions)
  ...
  assert.equal(
    observations.every(({ diagnostics }) => diagnostics <= 1),
    true
  )
})
...
test("HTTP failure diagnostics preserve exact Causes without leaking public details", async () => {
  ...
  await withServerLayer(
    ...
    async (handler) => {
      const response = await handler(
        subscriptionRequest("cause-probe", {
          toolsListChanged: true
        })
      )
...
test("live SSE encoding failure reports its first Cause without stranding termination", async () => {
  ...
  const app = streamToolLayer(toolName, () =>
    ...
    }).pipe(Effect.ensuring(Deferred.succeed(handlerCompleted, undefined).pipe(Effect.asVoid)))
  )
  ...
  await withServerLayer(
    ...
    async (handler) => {
      ...
      const observed = await promptOutcome(Effect.runPromise(Deferred.await(sinkEntered)), 500)
      assert.equal(observed._tag, "Response")

      const termination = await promptOutcome(
        reader.read().then(
          () => ({ _tag: "Resolved" }),
          (cause) => ({ _tag: "Rejected", cause })
        ),
      ...
      assert.equal((await promptOutcome(Effect.runPromise(Deferred.await(handlerCompleted)), 500))._tag, "Response")
    }
  )
})
...
test("Effect Platform Layer disposal closes subscription and pending ordinary streams", async () => {
  await withEffectPlatform(options({ enableJsonResponse: false }), async (handler, runtime) => {
    ...
    const subscription = makeSseCursor(
      await handler(
        subscriptionRequest("platform-dispose", {
          toolsListChanged: true
        })
      )
    )
...
test("subscription encoding failure closes ownership and publish interruption stays interrupted", async () => {
  ...
  await withServerLayer(probe.layer, options({ maxPendingFrames: 1 }), async (handler) => {
    ...
    const cyclic = {}
    cyclic.self = cyclic
    const invalidPublishes = await promptOutcome(
      Promise.all([
        Effect.runPromise(
          probe.service().publish({
            tag: "notifications/tools/list_changed",
...
test("queue-full subscription failure still terminates after publisher interruption", async () => {
  ...
  await withServerLayer(probe.layer, options({ maxPendingFrames: 1 }), async (handler) => {
    const response = await handler(
      subscriptionRequest("queue-full-failure", {
        toolsListChanged: true
      })
    )
    const cursor = makeSseCursor(response)
    ...
    cyclic.self = cyclic
    const abort = new AbortController()
    const invalidExit = Effect.runPromiseExit(
      subscriptionSink({
        tag: "notifications/tools/list_changed",
        payload: cyclic
      }),

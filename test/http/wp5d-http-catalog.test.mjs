...
test("empty cursor merges plans and tools-list-change clears before mismatch recovery", async () => {
  ...
  const frames = await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        const transport = yield* HttpClient.make({
          ...
          fetch: async (_input, init) => {
              ...
              const events = [
                {
                  ...
                  params: { _meta: subMeta, notifications: { toolsListChanged: true } }
                },
            ...
            if (!headers.has("mcp-param-fresh")) {
              return new Response(
                JSON.stringify({ jsonrpc: "2.0", id: body.id, error: { code: -32020, message: "refresh" } }),
                {
                  status: 400,
        ...
        yield* transport
          .request(request("before-change", "tools/call", { name: "echo", arguments: { value: "x" } }))
          .pipe(Stream.runDrain)
        yield* transport.request(request("replacement", "tools/list")).pipe(Stream.runDrain)
        yield* transport
          .request(request("after-replace", "tools/call", { name: "echo", arguments: { value: "x" } }))
          .pipe(Stream.runDrain)
        yield* transport
          .request(request("sub", "subscriptions/listen", { notifications: { toolsListChanged: true } }))
          .pipe(Stream.runDrain)
        return yield* transport
          .request(request("call", "tools/call", { name: "echo", arguments: { value: "x" } }))
  ...
  assert.equal(
    callAttempts[0].headers.get("mcp-param-empty-page"),
    "x",
    "own empty cursor must merge/replace the named plan"
  )
  ...
  assert.equal(
    calls.filter(({ body }) => body.method === "tools/list").length,
    4,

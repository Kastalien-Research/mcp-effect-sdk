...
The frozen vendor snapshot is the sole generation authority. Work package 3
removed the former raw copies under `src/generated/mcp/2026-07-28`;
deterministic Effect codecs and protocol facts under `src/generated/mcp/` are
now derived directly from the pinned files in `sources/vendor/mcp-core/`.
...
## SEP-1730 tiering authority

`sources/vendor/sep-1730/1730-sdks-tiering-system.md` is pinned at
`modelcontextprotocol/modelcontextprotocol` revision
`7634684382c3d14cf7e9f14073fe40a2d8ace3fa`. It defines the Tier 1, Tier 2, and
Tier 3 criteria that `docs/sdk-readiness-requirements.md` and
`scripts/check-sdk-readiness-requirements.mjs` encode as `GR-TIER-*`,
`GR-CONF-001`, `GR-REL-001`, and `GR-DOC-*` rows. It is a governance input, not
a protocol or runtime input: nothing generated or executed reads it.

Before this pin, every one of those rows cited
`../modelcontextprotocol/seps/1730-sdks-tiering-system.md` — a path outside the
repository that no checkout resolves. The Tier criteria the readiness gate
enforced could not be read, diffed, or audited from a clean clone, and a change
upstream would have gone unnoticed. Pinning it closes that gap.

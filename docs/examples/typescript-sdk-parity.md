...
The executable catalog is in `examples/typescript-sdk-ports/diagnostics.ts`. It
intentionally records partial and blocked ports instead of hiding missing SDK
seams behind example workarounds.
...
3. The generated schema contains `InputRequiredResult`, but the high-level tool
   registration result type and resolver reject it. Modern MRTR examples stop at
   that boundary.

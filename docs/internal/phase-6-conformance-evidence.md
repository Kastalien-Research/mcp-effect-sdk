...
Implement Phase 6: turn SEP-1730 SDK tier requirements into reproducible package
evidence, backed by conformance scenario mapping and an Everything-style server
...
- New task-runtime semantics beyond defects discovered while wiring conformance
  evidence.
...
| ------ | --------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-6.1 | The package includes an Everything-style example server for conformance testing.                                                        | `examples/everything-server.ts`, package script to run the built server, and docs naming the server URL/transport.                                       |
...
1. `docs/internal/acceptance-gates/sdk-generator.md`
...
- `ROADMAP.md`, only if current-state or remaining-work text would otherwise
  drift.
- `docs/internal/acceptance-gates/sdk-generator.md`, if Phase 8 is collapsed
  into this Phase 6 cleanup decision.

Do not edit:

- `src/generated/mcp/**`, unless Phase 6 exposes a real generator input gap.
- `../conformance/**`, unless the conformance harness itself is proven wrong and
  the fix is explicitly scoped.
...
- `check:conformance-evidence`: runs
  `node scripts/check-conformance-evidence.mjs`.
...
- `conformance:run`: builds the package, starts the server, waits for readiness,
  runs the conformance server suite, and stops the server.
...
| --------- | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-6.1    | `pnpm run build`; `pnpm run conformance:server`; `scripts/check-conformance-evidence.mjs` | Fail if `examples/everything-server.ts` is missing, excluded from `tsconfig.json`, not emitted under `dist/examples/`, or not referenced by a package script.                                                                                  |
...
CI must not depend on a sibling `../conformance` checkout. For MCP `2026-07-28`,
`pnpm run verify` is the package-health gate, while `pnpm run conformance:run`
is the draft-targeted official conformance qualification path.

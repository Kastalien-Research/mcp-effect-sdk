...
| Authorization                                | `mcp-effect-sdk/auth/client`                           | `qs mcp login <server>`: full PKCE flow (loopback redirect + browser open), pre-registered → stored → CIMD → DCR resolution order, step-up retry on `insufficient_scope`, tokens stored `Redacted` via `KeyValueStore` |
...
`KeyValueStore.layerSchema` + `effect/Schema`):
...
- Approval decisions, MCP server registry, OAuth grants (values `Redacted`, file
  mode 0600).

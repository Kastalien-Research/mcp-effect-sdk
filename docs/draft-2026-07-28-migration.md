...
`pnpm run test:protocol-metadata` for parity and fail-closed mutation coverage.
...
- Migrated the client path:
  - `McpClient` — no handshake; calls `server/discover`; attaches per-request
    ...
    request/notification surface; adds `subscriptionsListen` and `discover`.
...
- Kept only the marked Roots, Sampling, and Logging hooks under
  ...
  `InputRequiredPolicy` and `requestInput`; no deprecated Elicitation service or
  standalone server-request route remains.
- Added packed-consumer/root-export guards, cumulative `test:http` and
  `test:transports` gates, and frozen-draft parity/deferred-ledger checks.
...
- Added cache metadata and low-risk draft follow-ups:
  ...
  - Missing resources now fail with the draft `InvalidParams` code `-32602`; the
    old `-32002` resource-not-found code is not emitted as active draft
    behavior.
  - Tool `inputSchema`/`outputSchema` preserve JSON Schema 2020-12 keywords, and
    `structuredContent` may carry any decoded JSON value.
- Added the stable MRTR boundary:
  - `InputRequiredPolicy` automatically handles generated sampling, roots, and
    ...
    callers may lower those limits. URL Elicitation requires an explicit handler
    and is never fetched or opened by the SDK.
  ...
  - `SecureRequestState` seals opaque state with caller-provided 32-byte key
    ...
    one-time consumption through the explicit `RequestStateReplayStore` service.
    Use this for security or business state.
...
- Re-authored every active core example through published entrypoint owners. The
  core catalog includes stable form Elicitation through MRTR and the scoped
...
- Added direct focused `test:wp5-*` commands and the authoritative cumulative
  `pnpm run test:core` gate, including public-type and real-tarball consumer
  proof.
...
2. **Authorization hardening** — implemented locally through the stable
   ...
   conformance gate.
3. **Conformance + tasks examples** — the Everything, core protocol catalog, and
   agent-facing proof examples are draft-aligned and compile. The active

...
export const isTokenEndpointAuthMethod = (value: unknown): value is TokenEndpointAuthMethod =>
  value === "none" || value === "client_secret_post" || value === "client_secret_basic"
...
const snapshotAdvertisedMethods = (value: unknown): ReadonlyArray<string> | undefined => {
...
export const selectTokenEndpointAuthMethod = (
  ...
  if (configuredMethod !== undefined) {
    if (
      !isTokenEndpointAuthMethod(configuredMethod) ||
      !isTokenEndpointAuthMethodCompatible(configuredMethod, hasClientSecret)
    )
      return undefined

...
import type { AuthorizationStoreError } from "./errors.js"
...
import { type AuthorizationEndpointPolicy, isAllowedAuthorizationIssuer } from "./uri.js"
  ...
  try {
    for (const item of snapshot.values) {
      ...
      if (
        ...
        !("value" in descriptor) ||
        !isAllowedAuthorizationIssuer(descriptor.value, endpointPolicy)
      )
        return undefined
...
> =>
  Effect.gen(function* () {
    ...
    const configuredIssuers = snapshotPreRegistrationIssuers(input.preRegisteredCredentials, endpointPolicy)
...
const appendScopes = (output: Array<AuthorizationScope>, seen: Set<string>, scopes: AuthorizationScopeSet): void => {
...
export const resolveAuthorizationScopes = (input: ResolveAuthorizationScopesInput) =>
  Effect.gen(function* () {
    ...
    if (
      output.length === 0 &&
      input.challengeScopes === undefined &&
...
export const resolveAuthorizationContext = (input: ResolveAuthorizationContextInput) =>
  Effect.gen(function* () {
    const endpointPolicy = input.endpointPolicy ?? "https-only"
    ...
    const protectedResource = yield* discoverProtectedResourceMetadata({
      ...
      endpointPolicy,
      ...(input.resourceMetadataUri === undefined ? {} : { resourceMetadataUri: input.resourceMetadataUri })
    })
    ...
    const authorizationServerMetadata = yield* discoverAuthorizationServerMetadata(selected.issuer, endpointPolicy)
    ...
    const credentialHandle = yield* resolveAuthorizationCredential({
      issuer: selected.issuer,
      authorizationServerMetadata,
      ...(selectedCredentialHandle === undefined ? {} : { selectedCredentialHandle }),
      scopes,
      configuration,
      endpointPolicy
    })

...
const containCallback = <A, E, R>(
  ...
  containSchemaCallback(thunk, (cause) => schemaError(message, phase, cause))

const makeResolver = <R, E>(
  options: JsonSchemaResolverOptions<R, E>
): Effect.Effect<JsonSchemaResolverService, SchemaValidationError, R> =>
  Effect.gen(function* () {
...
const normalizeResolverPolicy = (value: Readonly<Record<string, unknown>>): JsonSchemaResolverPolicy => {
  ...
  return Object.freeze({
    ...
    maxRedirects: nonNegativeInteger(value.maxRedirects ?? DEFAULT_BUDGET.maxRedirects, "maxRedirects"),
...
export const snapshotJsonSchemaResolverService = (value: unknown): JsonSchemaResolverService => {
  ...
  return Object.freeze({
    ...
    resolve: (uri: string) =>
      containCallback(
        () => Reflect.apply(resolve, value, [uri]) as Effect.Effect<ResolvedJsonSchemaBytes, SchemaValidationError>,
        "JSON Schema resolver failed",
        "resolution"
      )
  })
...
const stringArray = (value: unknown, name: string): Array<string> => {
  ...
  if (snapshot === invalidStrictJson || !Array.isArray(snapshot) || snapshot.some((item) => typeof item !== "string")) {
...
const normalizeResolvedBytes = (value: unknown): ResolvedJsonSchemaBytes => {
  ...
  const bytes =
    typeof snapshot.bytes === "object" && snapshot.bytes !== null
...
): Effect.Effect<JsonSchema, SchemaValidationError> =>
  Effect.try({
    try: () => {
      const snapshot = cloneStrictJson(value)
      if (
        snapshot === invalidStrictJson ||
...
): Effect.Effect<ReadonlyArray<ResolvedDocument>, SchemaValidationError> =>
  Effect.gen(function* () {
    ...
    while (queue.length > 0) {
      ...
      const response = yield* resolver
        .resolve(next.uri)
        .pipe(Effect.flatMap((value) => resolutionTry(() => normalizeResolvedBytes(value))))
      yield* resolutionTry(() => {
      ...
      const aliases = [...response.redirects, response.finalUri, ...(base === undefined ? [] : [base])]
        .map(documentUri)
...
const resolutionTry = <A>(thunk: () => A): Effect.Effect<A, SchemaValidationError> =>
  Effect.try({
    try: thunk,
    catch: (cause) =>
      cause instanceof SchemaValidationError ? cause : schemaError("JSON Schema resolution failed", "resolution", cause)
  })

const decodeResolvedSchema = (bytes: Uint8Array): Effect.Effect<JsonSchema, SchemaValidationError> =>
  Effect.try({
...
const canonicalByteLength = (schema: JsonSchema): number => textEncoder.encode(JSON.stringify(schema)).byteLength
...
const referencesIn = (schema: JsonSchema, base: string | undefined, currentDocument: string): Array<string> => {
  const references: string[] = []
  const visit = (value: JsonSchema, inheritedBase: string | undefined, inheritedDocument: string) => {
...
const compileAjv = (root: JsonSchema, documents: ReadonlyArray<ResolvedDocument>): ValidateFunction => {
...
const validateValue = (validate: ValidateFunction, value: unknown): Effect.Effect<void, SchemaValidationError> =>
  Effect.gen(function* () {
    const snapshot = yield* Effect.try({
    ...
    return yield* Effect.fail(
      schemaError("JSON Schema validation failed", "instance", undefined, { issues: normalizeIssues(validate.errors) })
    )
  })

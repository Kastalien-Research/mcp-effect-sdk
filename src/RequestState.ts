...
const snapshotOwnData = (
  ...
  label: string
): Effect.Effect<Readonly<Record<string, unknown>>, RequestStateError> =>
  Effect.try({
    try: () => {
      ...
      return RequestStateReplayStore.of({
        consume: ({ nonce, expiresAt, now }) =>
          Ref.modify(entries, (current) => {
            ...
            if (live.has(nonce)) {
              return [
                new RequestStateError({
                  reason: "Replay",
                  message: "Request-state token was already consumed"
                }),
                live
              ] as const
            }
            if (live.size >= capacity) {
              return [
                new RequestStateError({
                  reason: "ReplayStoreFull",
                  message: "Replay store is full"
                }),
                live
              ] as const
            }
...
export class SecureRequestState extends Context.Service<SecureRequestState, SecureRequestStateService>()(
  ...
  static make(
    ...
    return Effect.gen(function* () {
      const snapshot = yield* snapshotOwnData(
        options,
        new Set(["key", "ttlMs", "now"]),
        "InvalidConfiguration",
        "secure request-state options"
      )
      ...
      return yield* Effect.gen(function* () {
        ...
        const cryptoKey = yield* Effect.tryPromise({
          try: () => crypto.subtle.importKey("raw", copied, { name: "AES-GCM" }, false, ["encrypt", "decrypt"]),
          catch: (cause) => requestStateError("InvalidConfiguration", "Could not import request-state key", cause)
        })
        const now = (nowValue as (() => number) | undefined) ?? Date.now

        const seal: SecureRequestStateService["seal"] = (input) =>
          Effect.gen(function* () {
            const inputSnapshot = yield* snapshotOwnData(
              input,
              new Set(["state", "principal", "purpose"]),
              "InvalidInput",
              "request-state seal input"
            )
            ...
            if (
              typeof state !== "string" ||
              !isWellFormedString(state) ||
              encoder.encode(state).byteLength > MAX_STATE_BYTES
            ) {
              return yield* requestStateFailure(
                "InvalidInput",
                "Request state must be a string of at most 8192 UTF-8 bytes"
              )
            }
        ...
        const open: SecureRequestStateService["open"] = (input) =>
          Effect.gen(function* () {
            const inputSnapshot = yield* snapshotOwnData(
              input,
              new Set(["token", "principal", "purpose"]),
              "InvalidInput",
              "request-state open input"
            )
            const principal = yield* binding(inputSnapshot["principal"], "principal")
            const purpose = yield* binding(inputSnapshot["purpose"], "purpose")
            ...
            const plaintext = yield* Effect.tryPromise({
              ...
              catch: (cause) => requestStateError("AuthenticationFailed", "Request-state authentication failed", cause)
            })
...
interface CryptoLike {
  ...
  readonly subtle: {
    ...
    readonly encrypt: (algorithm: AesGcmAlgorithm, key: unknown, data: Uint8Array) => Promise<ArrayBuffer>
    readonly decrypt: (algorithm: AesGcmAlgorithm, key: unknown, data: Uint8Array) => Promise<ArrayBuffer>
  }
}
...
const webCrypto = (): Effect.Effect<CryptoLike, RequestStateError> =>
  Effect.try({
    try: () => {
...
const currentTime = (now: () => number): Effect.Effect<number, RequestStateError> =>
  Effect.try({
    try: () => {
      const value = now()
...
const parseEnvelope = (bytes: Uint8Array): Effect.Effect<Envelope, RequestStateError> =>
  Effect.try({
    try: () => {
      ...
      if (
        ...
        decodeBase64Url(record.n)?.byteLength !== NONCE_BYTES ||
        typeof record.state !== "string" ||
        !isWellFormedString(record.state) ||
...
const requestStateError = (reason: RequestStateErrorReason, message: string, cause: unknown): RequestStateError => {

...
export interface PaginationCursorService {
  ...
  readonly invalidate: (collections?: ReadonlyArray<PaginatedCollection>) => Effect.Effect<void, SchemaValidationError>
}
...
): Effect.Effect<PaginationCursorService, SchemaValidationError> =>
  Effect.gen(function* () {
    ...
    const issue: PaginationCursorService["issue"] = (state) =>
      Effect.gen(function* () {
    ...
    const resolve: PaginationCursorService["resolve"] = (cursor) =>
      Effect.gen(function* () {
    ...
    const invalidate: PaginationCursorService["invalidate"] = (collections) =>
      Effect.try({
        try: () => {
          ...
          for (let index = 0; index < length.value; index++) {
            ...
            if (
              ...
              (descriptor.value !== "tools" &&
                ...
                descriptor.value !== "resourceTemplates" &&
                descriptor.value !== "prompts")
            ) {
              throw new TypeError("Invalid pagination cursor invalidation selector")
            }
...
export const normalizePaginationPolicy = (value: unknown): NormalizedPaginationPolicy => {

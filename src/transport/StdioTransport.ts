...
const framingError = (stage: StdioTransportStage, message: string, cause?: unknown): StdioTransportError =>
...
const consumeChunk = (
  ...
  Effect.gen(function* () {
    ...
    while (offset < chunk.byteLength) {
      ...
      if (newline === -1) {
        const last = segment.byteLength > 0 ? segment[segment.byteLength - 1] : buffered[buffered.byteLength - 1]
        if (combinedLength > maxLineBytes + 1 || (combinedLength === maxLineBytes + 1 && last !== carriageReturn)) {
          return yield* Effect.fail(framingError("FrameTooLarge", "Stdio frame exceeds maxLineBytes"))
        }
...
export const decode = <E, R>(
  ...
  const validatedMax = Number.isSafeInteger(maxLineBytes) && maxLineBytes >= 0 ? maxLineBytes : DEFAULT_MAX_LINE_BYTES
...
export const makeWriter = <WriteError, CloseError = never>(options: {
  ...
  Effect.gen(function* () {
    ...
    const close = semaphore.withPermits(1)(

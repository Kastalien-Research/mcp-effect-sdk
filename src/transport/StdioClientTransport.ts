  ...
  Effect.callback((resume) => {
    ...
    try {
      child.stdin.write(bytes, (cause) =>
        resume(cause ? Effect.fail(transportError("Write", "Could not write child stdin", cause)) : Effect.void)
      )
    } catch (cause) {
...
const toClose = (error: StdioTransport.StdioTransportError): StdioTransport.StdioTransportClose =>
...
export const make = (
  options: StdioClientTransportOptions
): Effect.Effect<McpTransport<StdioClientTransportError>, StdioClientTransportError, Scope.Scope> =>
  Effect.gen(function* () {
    ...
    const publishClose = (close: StdioTransport.StdioTransportClose): Effect.Effect<void> =>
      Deferred.succeed(closeSignal, close).pipe(
        Effect.flatMap((won) =>
          won
    ...
    const sendMessage = (message: JsonRpcMessage): Effect.Effect<void, StdioTransport.StdioTransportError> =>
      writer.send(message).pipe(Effect.tapError((error) => publishClose(toClose(error))))
    ...
    const inbound = StdioTransport.decode(readable(child, "stdout"), {
      ...
      Stream.runForEach((message) =>
        dispatcher
          .accept(message)
          .pipe(Effect.mapError((cause) => transportError("Protocol", "Unsupported inbound stdio message", cause)))
      ),
